...
# multiple users
def group_perturbation(df,timestamp,epsilon,w):

    n = len(df)
    groups = split_users(n,w)
    all_noise_result = []
    for i in range(0, len(timestamp), w):
        timestamp_block = timestamp[i:i + w]
        noise_result_g = []
        for g,group in enumerate(groups):
            df = df[group[0]:group[1]]
            sum_g = 0
            for j in range(0,len(df)):
                data = df[timestamp_block].values.tolist()
                sample_value = data[g]
                noise_value = Piecewise_mechanism(sample_value, epsilon)
                sum_g += noise_value
            noise_result_g.append(sum_g)
        all_noise_result.append(noise_result_g)
    all_noise_result = sum(all_noise_result,[])

    return all_noise_result

# single user
def sliding_window_sample_single(data, epsilon, w):
    """
    只采样一个数据，剩余数据使用采样数据的噪声值
    滑动窗口版本：在任意长度为 w 的窗口内只采样一个数据。保证任意窗口满足epsilon-LDP
    未采样到的位置复用最近采样的扰动值。
    """
    all_noise_result = []
    last_sample_index = 0  # 固定第0个位置采样

    for t in range(0, len(data)):
        if t == 0:
            # 对第0个位置进行加噪
            noise_value = Piecewise_mechanism(data[0], epsilon)
            last_noise_value = noise_value
        else:
            # 若距离上一次采样位置已满w个，则重新采样
            if t - last_sample_index >= w:
                sample_value = data[t]
                noise_value = Piecewise_mechanism(sample_value, epsilon)
                last_noise_value = noise_value
                last_sample_index = t
...
def perturb_value(values_t,domain, epsilon_t):
    ...
    if len(domain) < 3 * np.e ** (epsilon_t) + 2:
        ...
        count_vector_noise = np.zeros(len(domain))
        for cat, cnt in category_count_noise.items():
            idx = cat2idx[cat]
            count_vector_noise[idx] = cnt
...
def sliding_window_sample_multi(df, timestamp, epsilon, w, domain, column):
    ...
    for t in range(0, len(timestamp)):
        ...
        if t == 0:
            ...
            count_vector_noise = perturb_value(values_t, domain, epsilon)
...
def mean_square_error_single(round, df, epsilon, w):
    ...
    for r in range(0, round):
         all_noise_result = sliding_window_sample_single(input_data, epsilon, w)
...
def mean_square_error_multi(round, df, epsilon, w):
    ...
    for r in range(0, round):
        mse = 0
        noise_result = sliding_window_sample_multi(df, timestamp, epsilon, w, domain, column)
        for t in range(0,len(timestamp)):
            real = np.array(list(real_result[t]))/len(UID)
...
def mae_rangequery_multi(df, epsilon, w):
    ...
    MAE = 0
    noise_result = sliding_window_sample_multi(df, timestamp, epsilon, w, domain, column)
    queries = generate_fixed_length_queries(domain_min, domain_max, query_len=5, query_num=1000, seed=None)
