...
def LBA_single(data,epsilon_1, epsilon_2, w):
    ...
    for t in range(len(data)):
        ...
        if t == 0 :
            ...
            # 计算多少时间戳需要冻结
            t_N = last_eps / (epsilon_2 / w) - 1
            # 判断当前时间 t 是否仍处于预算冻结期。
            if t - last_pub_time <= t_N:
                noise_data = noise_result_2[last_pub_time]
                noise_result_2.append(noise_data)
            else:
                # 吸收多少时间戳的隐私预算
                t_absorb = t - (last_pub_time + t_N)
                epsilon_t = (epsilon_2 / w) * min(t_absorb, w)
                # 计算dis
...
def perturb_M1_categorical(df, timestamp, domain, epsilon_1, w, column):
    ...
    for t in tqdm(range(len(timestamp))):
        ...
        all_noise_result.append(count_vector_noise)
    return all_noise_result

def LBA_multi(df, timestamp,epsilon_1, epsilon_2, domain, w, column):
...
def mean_square_error_single(round, df, epsilon, w):
    ...
    for r in range(0, round):
         all_noise_result = LBA_single(input_data,epsilon_1, epsilon_2, w)
...
def mean_square_error_multi(round, df, epsilon, w):
    ...
    for r in range(0, round):
        mse = 0
        noise_result = LBA_multi(df, timestamp,epsilon_1, epsilon_2, domain, w, column)
